...
country_codes <- read_csv("./data/raw/world_country_codes.csv")

primary_energy_clean <-
  primary_energy %>% 
  filter(year %in% 1990:2015) %>% 
  left_join(country_codes, by = c("country_iso3" = "a3_code")) %>% 
  select(-a2_code, -num_code)
...
primary_energy_clean %>% filter(year %in% 1991:1992) %>% View
primary_energy %>% filter(year %in% 1991:1992) %>% View
